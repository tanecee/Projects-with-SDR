...
std::vector<std::complex<float>> generateOFDMSignal(const OFDMParameters& p, const std::string& message, size_t& actual_Nsym) {
    ...
    // Mesaj için gerekli sembol sayısını hesapla
    size_t required_data_symbols = static_cast<size_t>(std::ceil(static_cast<double>(total_bits) / 2));
    actual_Nsym = static_cast<size_t>(std::ceil(static_cast<double>(required_data_symbols) / p.dataScs));

    // Nsym bilgisini 16 bit olarak kodla (max 65535 sembol)
    std::bitset<16> nsym_bits(actual_Nsym);
    std::vector<int> nsym_bit_vector;
    for(int i = 15; i >= 0; i--) {
        nsym_bit_vector.push_back(nsym_bits[i]);
    }
    
    // Nsym ve mesaj bitlerini birleştir
    std::vector<int> combined_bits = nsym_bit_vector;
    combined_bits.insert(combined_bits.end(), tx_bits.begin(), tx_bits.end());
    
    // Toplam sembol sayısını yeniden hesapla
    size_t total_required_data_symbols = static_cast<size_t>(std::ceil(static_cast<double>(combined_bits.size()) / 2));
    size_t final_Nsym = static_cast<size_t>(std::ceil(static_cast<double>(total_required_data_symbols) / p.dataScs));
    
    std::cout << "Gonderilecek OFDM sembol sayisi (Nsym): " << final_Nsym << std::endl;
    std::cout << "Mesaj bit sayisi: " << tx_bits.size() << std::endl;
    std::cout << "Nsym bilgisi icin bit sayisi: " << nsym_bit_vector.size() << std::endl;
    
    // Toplam bit sayısını son sembol sayısına göre ayarla
    size_t required_bits = final_Nsym * 2 * p.dataScs;
    if (combined_bits.size() < required_bits) {
        combined_bits.resize(required_bits, 0); // Kalanı sıfırla doldur
    }

    auto qam_symbols = qpskMod(combined_bits);

    // OFDM grid'i oluştur
    std::vector<std::complex<float>> ofdmGrid(p.Nfft * final_Nsym, 0.0f);
    
    for (size_t symbol_idx = 0; symbol_idx < final_Nsym; symbol_idx++) {
    ...
    size_t wformLength = (p.Nfft + p.cpLength) * final_Nsym;
