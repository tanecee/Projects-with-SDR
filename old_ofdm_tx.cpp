...
// FFT shift (merkezde DC olacak şekilde)
...
std::vector<std::complex<float>> generateOFDMSignal(const OFDMParameters& p) {
    ...
    // Zaman domain sinyali oluşturma
...
int UHD_SAFE_MAIN(int argc, char* argv[]) {
    ...
    // OFDM parametreleri
    OFDMParameters p;
    double rate = p.sample_rate; // OFDM sample rate'i kullan
    double freq = 3.1e9;
    double gain = 20;     
    ...
    const size_t spb = ofdmSignal.size(); // Tüm OFDM sinyalini buffer olarak kullan
    ...
    try {
        while (true) {
            // Zaman senkronizasyonu için bekle
