...
baseThr = 0.15;
    ...
    try
        while true
            ...
            % Senkronizasyon sinyali bulunduysa
            if ~found && ~isempty(pks)
                % Ek güvenlik kontrolü: Tepe/Ortalama oranı yeterli mi?
    ...
    % Temizlik: PlutoSDR nesnesini serbest bırak
    release(rx);
    
    % Senkron bulunamadıysa, dosya yolu boş dönsün
    if ~found
        rx_file = '';
    end
end
