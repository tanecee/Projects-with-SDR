...
OFDMParameters::OFDMParameters() {
    // --- mevcut index üretimi ---
    for (size_t i = 1; i <= 2 * dataScs; i += 2) {
        dataInd.push_back(i + Nfft / 4);   // 65,67,...,191 (0-bazlı)
    }
    for (size_t i = 2; i <= 2 * pilotScs; i += 2) {
        pilotInd.push_back(i + Nfft / 4);  // 66,68,...,192 (0-bazlı)
    }

    // === DC’yi boş bırak ===
    const size_t DC = Nfft/2; // 0-bazlı merkez (fftshift’li gridde)
    pilotInd.erase(std::remove(pilotInd.begin(), pilotInd.end(), DC), pilotInd.end());
    dataInd.erase (std::remove(dataInd.begin(),  dataInd.end(),  DC), dataInd.end());

    // Güncel sayılar (DC çıktıktan sonra)
    pilotScs = pilotInd.size();
    dataScs  = dataInd.size();

    // === Pilot işaretleri (DC çıkınca uzunluk güncellendi) ===
    pilot1.assign(pilotScs, { });
    pilot2.assign(pilotScs, { });
...
std::vector<std::complex<float>> IFFT(const std::vector<std::complex<float>>& X) {
    ...
    for (size_t n = 0; n < N; ++n) {
        ...
        x[n] = sum / static_cast<float>(N); // <-- normalize
    }
    return x;
}
...
std::vector<std::complex<float>> generateOFDMSignal(const OFDMParameters& p) {
    ...
    for (size_t symbol_idx = 0; symbol_idx < p.Nsym; symbol_idx++) {
        ...
        for (size_t j = 0; j < p.pilotScs; j++) {
            ofdmGrid[symbol_idx * p.Nfft + p.pilotInd[j]] = pilot[j];
        }
        
        for (size_t j = 0; j < p.dataScs; j++) {
            ofdmGrid[symbol_idx * p.Nfft + p.dataInd[j]] = 
    ...
    for (size_t symbol_idx = 0; symbol_idx < p.Nsym; symbol_idx++) {
        ...
        /*// Ölçekleme (sqrt(Nfft))
        for (auto& sample : ifft_output) {
            sample *= std::sqrt(static_cast<float>(p.Nfft));
        }*/
